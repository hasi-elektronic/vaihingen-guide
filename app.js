...
const ICONS = {gastro:'🍽️',einkaufen:'🛍️',dienstleistung:'🔧',kultur:'🏛️',natur:'🌿',aktiv:'🚴'};
...
function cardHTML(p){
  const im = imgs(p);
  const ph = im.length ? `<img class="ph" loading="lazy" src="${API+im[0]}" alt="${p.name}">`
                       : `<div class="ph">${ICONS[p.category]||'📍'}</div>`;
  return `<a class="card" href="ort.html?id=${p.slug||p.id}">
    ${ph}<div class="body">
    <span class="cat">${t('cat_'+p.category)}</span>
    <h3>${p.name}</h3><p>${desc(p)}</p>
...
function initMap(el, places){
  ...
  places.forEach(p=>{ if(p.lat&&p.lng){
    ...
    L.marker([p.lat,p.lng]).addTo(map)
     .bindPopup(`<b>${p.name}</b><br><a href="ort.html?id=${p.slug||p.id}">${t('all').replace('Alle anzeigen','Details').replace('Show all','Details')} →</a>`);
  }});
...
async function pageList(type, cats){
  ...
  async function load(){
    ...
    if(mapEl){ mapEl.innerHTML=''; if(mapEl._m){mapEl._m.remove()} mapEl._m = initMap(mapEl, rows); }
  }
